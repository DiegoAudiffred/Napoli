...
class modifyEmployeeForm(UserChangeForm):
    nueva_contraseña = forms.CharField(widget=forms.PasswordInput)
    class Meta:
        model = User
        fields = ['email','first_name','phone_number','url', 'rol',]


    def __init__(self, *args, **kwargs):
        super(modifyEmployeeForm, self).__init__(*args, **kwargs)
        self.fields['nueva_contraseña'].required = False
        self.fields['nueva_contraseña'].widget.attrs.update({'class':'form-control shadow-none bg-corporateTan200  px-2 py-1 text-primary border border-0','placeholder':' Contraseña','rows':'1', 'aria-label':'Username','aria-describedby':'basic-addon1','style':'border-left:none', })
